...
The examples used for the `response_examples` variable are loaded from the `in_context_examples.csv` file in the `/custom_components/home_llm_router/` folder. For different language add extra suffix to name of this file eg. for polish responses set `in_context_examples_pl.csv` (note: currently only polish language is supported).
